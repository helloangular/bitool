(ns bitool.compiler.dialect.bigquery
...
(defn- group-by-list
  ...
  (let [group-cols (set (:group_by sql-ir))]
    ;; `:group_by` is expressed in terms of target columns, and the compiler keeps
    ;; those aligned with the SELECT expressions we emit here.
    (->> (:select sql-ir)
...
(defn compile-materialization-sql
  ...
  (let [{:keys [mode target keys]} (:materialization sql-ir)
    ...
    (case mode
      ...
      (throw (ex-info "Unsupported materialization mode for BigQuery compilation"
                      {:mode mode
                       :sql_ir sql-ir})))))
