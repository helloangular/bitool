...
(defn add-mapping [ g ] (add-node g (create-node "mapping" "M")))
...
(defn top-sort-all [ g ] (alg/topsort (graph/digraph (into {} (map (fn [[k v]] [k (vec (keys (:e v)))]) (:n g))))))
...
(defn find-edges [ g node nodeType ] 
...
(defn find-node-by-name [ g name ] 
  (->> (get g :n)
       (filter (fn [[k v]]
		 (and (= k :name)
                      (= name (get-in v [:na :name])))))))

(defn create-column [params]
  (conj (conj [] (:id params)) (mapv params [:technical_name :data_type :nullable :length :precision :scale :key :expression :transform :af :exclude])))
...
(defn get-mapping-node[g t2]
      (first (filter #(= (btype % g) "mapping") (children g t2))))
...
(defn map-tcols[t2 t2-table g]
      (let [mnode (get-mapping-node g t2)]
           (if mnode (setMappingTarget g mnode t2-table) g)))
...
(defn get-agg-columns [columns]
  (mapv #(select-keys % [:tid :technical_name :function]) 
     (first (sp/select [(sp/filterer #(some? (:function %)))] columns))))

(defn update-agg-column[cols col]
    (sp/setval  [sp/ALL #(and (= (first %) (:tid col)) (= (first (second %)) (split_dot_second (:technical_name col)))) (sp/nthpath 1) (sp/nthpath 9) ] (:function col) cols))
...
(defn update-having[g id having]
      (assoc-in g [:n id :na :having] having))

(defn update-groupby[g id columns]
      (assoc-in g [:n id :na :groupby] (get-groupby columns)))

(defn update-agg[g id params]
      (let [
             columns (:columns params)
           ]
      	   (update-agg-tcols (update-groupby (update-having g id (:having params)) id columns) id columns)))

(defn update-excluded-column[cols col]
    (sp/setval  [sp/ALL #(and (= (first %) (:tid col)) (= (first (second %)) (split_dot_second (:technical_name col)))) (sp/nthpath 1) (sp/nthpath 10) ] "YES" cols))

(defn update-excluded-columns[g id columns]
      (reduce #(update-excluded-column  %1 %2) (tcols g id) columns))

(defn update-excluded-tcols[g id columns]
      (assoc-in g [:n id :na :tcols] (update-excluded-columns g id columns)))


(defn update-rename-column[cols col]
    (sp/setval  [sp/ALL #(and (= (first %) (:tid col)) (= (first (second %)) (split_dot_second (:oldTechnicalName col)))) (sp/nthpath 1) (sp/nthpath 0) ] (split_dot_second (:newTechnicalName col)) cols))

(defn update-rename-columns[g id columns]
      (reduce #(update-rename-column  %1 %2) (tcols g id) columns))

(defn update-rename-tcols[g id columns]
      (assoc-in g [:n id :na :tcols] (update-rename-columns g id columns)))

(defn update-projection[g id params]
      (update-excluded-tcols (update-rename-tcols g id (:renamedColumns params)) id (:excludedColumns params)))
...
(defn nodes-to-update [ g node ] (find-nodes g ["output" "projection"] (successors g node)))
...
(defn update-table-cols [ g start-node table ]
      (update-nodes-tcols g (find-nodes g ["output" "projection"] (successors g start-node)) (:tcols table)))
					  ...
					  _ (println (str "t2-table" t2-table))
 					  ...
 					  _ (pp/pprint g2-proj)
 					    ...
 					    (unijoin-add-table join-t2 t2 jtype)
                                            (update-nodes-tcols [ proj-t2 ] (get-in g2-proj [:n t1 :na :tcols]))
					    (update-table-cols t2 prefix-table-map)
					    (db/insertGraph))))
	...
	(let [ t2-name (:name t2-table)
	       ...
	       prefix-table-map (prefix-table-columns g t2-table)
   	       _ (println (str "prefix-table-map" prefix-table-map))
               g2 (if (nil? c2) (add-node g prefix-table-map) (add-node g c2 prefix-table-map))
               _ (println  "g2" )
               _ (pp/pprint g2)
               t2 (nodecount g2)
               _ (println (str "t2" t2))
              ...
              (->> g2
                  (sp/multi-transform [:n tid :na (sp/multi-path [:c (sp/terminal-val c2)]
						      	      [:tcols (sp/terminal-val prefix-table-map)])])
...
(defn column_row_details [g table_id table_prefix row]
    ...
    {
     "tid" table_id
     "business_name"  (str table_prefix "." (nth row 0))
     "technical_name" (str table_prefix "." (nth row 0))
     "semantic_type"  (nth row 1)
     "data_type"      (nth row 1)
     "key"            (nth row 6)
     "column_type"    "column"
     "exp" 	      (nth row 7)
...
(defn item_master[id alias btype tmap]
      (let [
           ...
           (case btype 
  		 "aggregation" (assoc item "having" (:having tmap))
  		 "filter" (assoc item "where" (:sql tmap))
 		 item)))

(defn item_columns
  ([g columns] into [] (map #(column_details g %) columns))
  ([g columns table_id table_prefix] (map #(column_row_details g table_id table_prefix %) columns)))
...
(defn mapping_items[g tmap]
      ...
      (mapv #(assoc {} 	"source" (column_details g (get-by-table-column scols (Integer. (:sourceTid %)) (split_dot_second (:sourceColumn %))))
          		"target" (column_details g (get-by-table-column tcols (Integer. (:targetTid %)) (split_dot_second (:targetColumn %))))
...
(defn txunion[key-fn val-fn m]
  (into {} (map (fn [[k v]] [(key-fn k) (mapv val-fn v)]) m)))       
...
(defn get-unijoin-item[btype id g]
        (let [
            ...
            btype (:btype tmap)
               _ (println "TMAP")
               _ (println tmap)
            items (txunion #(node-name % g) #(column_row_details g id (node-name id g) %) ((keyword btype) tmap))
             ]
        ;;(assoc (item_master id alias btype) "tables" (mapv #(node-name % g) (:tables tmap)) "items" items)))
        (assoc (item_master id alias btype  tmap) "tables" (mapv #(node-name % g) (:tables tmap)) "join_items" (anil? (:join-items tmap) []) "items" (assoc items "Final" (-> items vals first)))))
...
(defn setMappingTcols[g mid sid tid]
      (-> g
          (assoc-in [:n mid :na :source] (tcols g sid))
          (assoc-in [:n mid :na :target] (tcols g tid))
		...
		mapping_id (nodecount g1)
 		edge (find-edge g1 t1 "src")
                ...
                g (-> g1
                      ...
                      (setMappingTcols mapping_id t1 target_id))
                cp (db/insertGraph g)
 		sp (getOrphanAttrs cp)
...
(defn save-conn [ args ]
    (let [ params (:json-params args)
           ...
           _ (pp/pprint args) ] 
    (def conn-id (:connection/id (db/insert-data :connection (sp/transform :port #(Integer/parseInt %) (walk/keywordize-keys (:json-params args))))))
                                    ...
                                    ;;   _ (println (str "latiEnd : " latiEnd))
                                       ...
                                       coord {}]
                                        (map #(assoc coord  :alias (:name (second %))
                                                            ...
                                                            :btype (:btype (second %))
                                                            :parent (if (nil? (parent gdef (first %))) 0 (parent gdef (first %)))
                                                            :id (first %)
                                                           ) attrs)))
