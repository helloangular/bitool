...
export function test(func, expect, message = 'The testcase was success!') {
  console.log(func() === expect ? '✅ ' + message : '❌ Failed ' + message)
}
...
export class FollowUpLine {
  ...
  start(fromEl, ev) {
    if (fromEl) {
      ...
      let fromX, fromY;
      if (edge === "left") {
        fromX = rect.left - this.svgBound.left; // left edge
        fromY = rect.top + rect.height / 2 - this.svgBound.top;
      } else {
        fromX = rect.right - this.svgBound.left; // right edge
        fromY = rect.top + rect.height / 2 - this.svgBound.top;
      }

      this.previewLine = document.createElementNS("http://www.w3.org/2000/svg", "line");
      this.previewLine.setAttribute("x1", fromX);
      this.previewLine.setAttribute("y1", fromY);
      this.previewLine.setAttribute("x2", fromX);
      this.previewLine.setAttribute("y2", fromY);
