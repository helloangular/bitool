...
export default class StateManager {
  ...
  /**
   * Updates a field in the current state, supporting simple assignment and array manipulation.
   * @param {string} key - The key (property name) in the state object to update.
   * @param {*} value - The value to assign or the item to push/pop.
   * @param {Object} [options] - Options defining the update behavior.
   * @param {('assign'|'push'|'pop')} [options.type='assign'] - The type of operation.
   * 'assign': Sets the field to the new value.
   * 'push': Pushes the value to the array at `key`.
   * 'pop': Removes the last element from the array at `key` (value is ignored).
   */
  updateField(key, value, options = { type: "assign" }) {
    if (!key || key.trim() === "") return;
    const { type } = options;
    if (type === "assign") this.#currentState[key] = value;
    if (type === "push") this.#currentState[key].push(value);
    if (type === "pop") this.#currentState[key].pop(); // value is not used in Array.prototype.pop()
  }
  ...
  getUpdatedFieldsWithRequiredFields(feilds) {
    return this.getUpdatedFields(this.#initialState, this.#currentState, feilds);
  }

  getUpdatedFields(initial, current, feilds = null) {
    let updatedFields = {};
    ...
    for (const [key, value] of Object.entries(current)) {
      if (initial[key] === undefined) {
        ...
        if (typeof initial[key][0] === 'object' && typeof current[key][0] === 'object') {
          const nestedUpdates = [];
