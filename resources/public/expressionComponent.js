...
<div>
  ...
  <div class="tabs-container">
    <div class="tab-header">
      <div class="tab active" data-tab="functions">Functions (0)</div>
      <div class="tab" data-tab="columns">Columns (0)</div>
      <div class="tab" data-tab="parameters">Parameters</div>
      <div class="tab" data-tab="other">Other</div>
    </div>
    <div class="tab-content">
      <div class="search-container">
        <select class="function-type-select" id="functionTypeSelect" >
...
class ExpressionComponent extends HTMLElement {
  ...
  initializeFunctionTypeSelect() {
    this.functionTypeSelect.selectedIndex = 0;
  }
  ...
  setupEventListeners() {
    ...
    EventHandler.on(this.searchInput, "input", this.handleFunctionSearch.bind(this));

    this.shadow.querySelectorAll("#otherContent .smart-button").forEach((button) => {
      EventHandler.on(button, "click", () => {
        this.insertExpression(button.textContent);
      });
    });

    EventHandler.on(this.functionTypeSelect, "change", this.handleFunctionTypeFilter.bind(this));
  }
  ...
  handleFunctionTypeFilter() {
    ...
    functionItems.forEach((item) => {
      ...
      const matchesType = selectedType === "All Functions" || functionType === selectedType;
  ...
  handleFunctionSearch(e) {
    ...
    functionItems.forEach((item) => {
      ...
      const matchesType = selectedType === "all" || functionType === selectedType;
  ...
  populateFunctions() {
    ...
    // display count of functions and columns in tab headers.
    this.shadowRoot.querySelector("[data-tab='functions']").textContent = `Functions (${this.functions ? Object.keys(this.functions).reduce((acc, key) => acc + this.functions[key].length, 0) : 0})`;
    this.shadowRoot.querySelector("[data-tab='columns']").textContent = `Columns (${this.selectedRectangle ? this.selectedRectangle.items.filter((item) => item.column_type === "column" || item.column_type === "measure").length : 0})`;

    Object.keys(this.functions).forEach(key => {
      this.functions[key].forEach((func) => {
        const functionItem = document.createElement("div");
        functionItem.className = "function-item";
        functionItem.setAttribute("data-type", key);
        functionItem.innerHTML = `
          <div class="function-name">${func.name} Function (${key})</div>
  ...
  showColumns() {
    ...
    const columns = this.selectedRectangle.items.filter(
      (item) => (item.column_type === "column" || item.column_type === "measure") && item.af !== "None"
    );

    this.shadowRoot.querySelector("[data-tab='columns']").textContent = `Columns (${columns.length})`;
    ...
    EventHandler.off(this.searchInput, "input", this.handleFunctionSearch);
    EventHandler.on(this.searchInput, "input", this.handleColumnSearch.bind(this));
  }
