...
class TransformEditor extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    if (name === "item") {
      ...
      if (this.item && newItem.technical_name !== this.item.technical_name) {
  ...
  addTransformation(button) {
    ...
    const transformList = parentDiv.nextElementSibling;
    ...
    listItem.textContent = `${transformType} (${parameters || "No parameters"}) `;
    ...
    transformList.appendChild(listItem);
  ...
  save() {
    const dataObj = {
      ...
      transformations: Array.from(this.shadowRoot.querySelectorAll(".transform-list li")).map(li => {
        return li.textContent.replace(" Remove", "");
      })
    }
