...
class TreeComponent extends HTMLElement {
		...
		try {
			...
			droppableArea.addEventListener("drop", (event) => {
				...
				if (event.target.tagName === "RECTANGLE-COMPONENT") {
					const table1 = event.dataTransfer.getData("Text");
					const table2 = event.target.getAttribute("alias");

					const { connectionName, schemaName } = window.data.connections.filter(
						(connection) =>
							connection.tables.map((table) => table.name).includes(table1)
					)[0];

					this.shadowRoot.getElementById("table1").value = table1;
					this.shadowRoot.getElementById("table2").value = table2;
					this.shadowRoot.getElementById("conn-id").value = connectionName;
					this.shadowRoot.getElementById("schema").value = schemaName;
	...
	handleDragStart(event) {
		...
		event.dataTransfer.setData("text/plain", draggedItem.label);
	}
	...
	async createConnection(item) {
		...
		try {
			...
			const addTableMetadata = (item, connId, schema) => {
				if (item.items && item.items.length > 0) {
					...
					item.items = item.items.map(subItem => addTableMetadata(subItem, connId, schema));
				}
				return item;
			};
			...
			window.data.treeItems = this.mapItems(
				...
				addTableMetadata(data["tree-data"], data["conn-id"], data["tree-data"]["items"][0]["label"])
			);
