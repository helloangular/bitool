...
class CustomWebComponent extends HTMLElement {
  ...
  initializeAttributes() {
    const attributes = [
      ...
      "info", "aggregation", "parent_object", "column_type", "association"
    ];
  ...
  updateBusinessName() {
    this.business_nameElements.forEach(element => element.textContent = this.business_name);
  }
  ...
  static get observedAttributes() {
    return [
      ...
      "association", "excluded", "draggable"
    ];
  }
  ...
  getProjectionMenuIcon() {
    if (this.column_type === "calculated") {
      return {
        ...
        dropDownElement: `
          <div class="tooltip-item">${this.getAttribute("excluded") === "true" ? "Include Column" : "Exclude Column"}</div>
  ...
  addTooltipClickListener() {
    EventHandler.on(this.dropDowntooltip, 'click', (e) => {
      if (e.target.textContent !== "Exclude Column" && e.target.textContent !== "Include Column") {
